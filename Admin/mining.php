...
function mining_process($db_object, $min_support, $min_confidence, $start_date, $end_date, $id_process){
    ...
    //build kandidat 1-itemset
    echo "<br><strong>Kandidat 1-itemset :</strong><br>";
    echo "<table class='table table-bordered table-striped  table-hover' style='text-align:center;'>
            <tr>
                <th><center>No</th>
                <th><center>Item</th>
                <th><center>Jumlah Item</th>
    ...
    foreach ($item_list as $key => $item) {
        ...
        if ($support>=$min_support_relative) {
            echo "<td style='color:#00ff00;'>" . price_format($support) . " %</td>";
        } 
        else {
            echo "<td style='color:#ff0000;'>" . price_format($support) . " %</td>";
        }
        echo "</tr>";
        $x++;
    }
    ...
    $sql_insert_itemset1 = "INSERT INTO itemset1 (item, jumlah, support, status, id) "
            . " VALUES ".$value_insert;
    $cek1 = $db_object->db_query($sql_insert_itemset1);
    
    //display Large 1-itemset
    echo "<br><strong>Large 1-itemset (Item Yang Memenuhi Minimal Support) :</strong><br>";
    echo "<table class='table table-bordered table-striped  table-hover' style='text-align:center;'>
            <tr>
                <th><center>No</th>
                <th><center>Item</th>
                <th><center>Suppport</th>
            </tr>";
    $x=1;
    ...
    //build kandidat 2-itemset
    echo "<br><strong>Kandidat 2-Itemset :</strong><br>";
    echo "<table class='table table-bordered table-striped  table-hover' style='text-align:center;'>
            <tr>
                <th><center>No</th>
                <th><center>Item1</th>
                <th><center>Item2</th>
                <th><center>Jumlah Kombinasi</th>
                <th><center>Suppport</th>
            </tr>";
            ...
            if (!empty($variance1) && !empty($variance2)) {
                if ($variance1 != $variance2) {
                    if(!is_exist_variasi_itemset($NilaiAtribut1, $NilaiAtribut2, $variance1, $variance2)) {
                        ...
                        if ($support2>=$min_support_relative) {
                            echo "<td style='color:#00ff00;'>" . price_format($support2) . " %</td>";
                        } 
                        else {
                            echo "<td style='color:#ff0000;'>" . price_format($support2) . " %</td>";
                        }
                        echo "</tr>";
                        $no++;
                    }
    ...
    $sql_insert_itemset2 = "INSERT INTO itemset2 (item1, item2, jumlah, support, status, id) "
            . " VALUES ".$value_insert_itemset2;
    $cek2 = $db_object->db_query($sql_insert_itemset2);

    //display Large 2-itemset
    echo "<br><strong>Large 2-itemset (Item Yang Memenuhi Minimal Support) :</strong><br>";
    echo "<table class='table table-bordered table-striped  table-hover' style='text-align:center;'>
            <tr>
                <th><center>No</th>
                <th><center>Item 1</th>
    ...
    $sql_insert_itemset3 = "INSERT INTO itemset3(item1, item2, item3, jumlah, support, status, id) "
    ...
    */
    ...
    $sql_3 = "SELECT * FROM itemset3 WHERE status = 1 AND id = ".$id_process;
    ...
    if($jumlah_itemset3_lolos > 0){
        ...
        while($row_3 = $db_object->db_fetch_array($res_3)){
            $atribut1 = $row_3['item1'];
            $atribut2 = $row_3['item2'];
            $atribut3 = $row_3['item3'];
    ...
    */
    ...
    //dari itemset 2
    $sql_2 = "SELECT * FROM itemset2 WHERE status = 1 AND id = ".$id_process;
    ...
    if($jumlah_itemset2_lolos > 0){
        ...
        while($row_2 = $db_object->db_fetch_array($res_2)){
            $atribut1 = $row_2['item1'];
            $atribut2 = $row_2['item2'];
...
function get_count_itemset3($db_object, $atribut1, $atribut2, $atribut3, $start_date, $end_date) {
    $sql = "SELECT COUNT(tanggal_transaksi) AS jml, tanggal_transaksi FROM transaksi 
            WHERE (produk='$atribut1' OR produk = '$atribut2'  OR produk = '$atribut3') 
                AND tanggal_transaksi BETWEEN '$start_date' AND '$end_date' 
            GROUP BY tanggal_transaksi
            HAVING COUNT(tanggal_transaksi)=3";
    $result = $db_object->db_query($sql);
    $jml = $db_object->db_num_rows($result);
    return $jml;
}
...
//    while($row1_ = $db_object->db_fetch_array($res1_)){
    ...
    $nilai_support_x = ($jml_itemset2/$jumlah_transaksi) * 100;
        ...
        $db_object->insert_record("kesimpulan", 
